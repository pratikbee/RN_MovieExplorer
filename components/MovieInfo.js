...
import {Add_Movie_Cast_Imdb_Id,Movie_Cast_Array,AddGenre} from '../actions/movieActionCreator';
import LFC from './LFC';
    ...
    <View style={{height:'100%'}}>
        <ImageBackground style={{height:'56%',width:'100%'}} source={{uri:props.route.params.results.banner}} blurRadius={1}>
           ...
           <View style={{marginTop:'10%',marginLeft:'2%'}}>
                ...
                <View><Text style={{marginTop:'2%',color:'#ff4d88',fontSize:30,fontWeight:'500'}}>{props.route.params.results.rating}*</Text><View style={{flexDirection:'row'}}>
                {[...Array(Math.round(props.route.params.results.rating/2))].map((elementInArray, index) =>
...
<View>
    ...
    renderItem={({item})=>(<View style={{height:150,width:100,padding:20}}>{item.results.image_url!=null?<Image style={{height:'70%',width:'90%',borderRadius:10}} source={{uri:item.results.image_url}}/>:<Image style={{height:'80%',width:'90%'}} source={require('/Users/ambin04957/Desktop/API/movieproject/Images/notavailaible.png')}/>}<View style={{alignItems:'center'}}><Text style={{fontSize:15,fontWeight:'400',position:'absolute'}}>{item.results.name}</Text></View></View>)}></FlatList>
    </View>
   <View style={{width:'100%',marginTop:'15%'}}><LFC data={props.route.params} movietype={true}></LFC></View>
